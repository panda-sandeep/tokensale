var PandaTokenCrowdsale = artifacts.require("PandaTokenCrowdsale");
var PandaToken = artifacts.require("PandaToken");

contract('PandaTokenCrowdsale', function(accounts) {
    it('should deploy the token and store the address', function(done){
        PandaTokenCrowdsale.deployed().then(async function(instance) {
    ...
    it('one ETH should buy 5 PandaTokens in PreICO', function(done){
        PandaTokenCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[7], value: web3.toWei(1, "ether")});
            const tokenAddress = await instance.token.call();
            const pandaToken = PandaToken.at(tokenAddress);
            const tokenAmount = await pandaToken.balanceOf(accounts[7]);
    ...
    it('one ETH should buy 2 PandaTokens in ICO', function(done){
        PandaTokenCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[2], value: web3.toWei(1.5, "ether")});
            const tokenAddress = await instance.token.call();
            const pandaToken = PandaToken.at(tokenAddress);
            const tokenAmount = await pandaToken.balanceOf(accounts[2]);
